...
if st.session_state.view == 'scanner':
    ...
    # --- SIDEBAR: ESTRATEGIA (CRITERIO DE VEREDICTO) ---
    st.sidebar.markdown("---")
    st.sidebar.header("⚙️ Estrategia de Análisis")
    st.sidebar.info("Estos parámetros definen el cálculo de 'Est. Fees' vs 'Max IL' para el veredicto.")
    
    dias_analisis = st.sidebar.slider("Ventana Análisis (Días)", 3, 30, 7, help="Días para proyectar fees y calcular volatilidad.")
    # Slider solicitado: 0.1 a 3.0 con saltos de 0.1
    ...
    if st.sidebar.button("Analizar Pool Concreto"):
        if manual_address:
            ...
            with st.spinner("Analizando dirección específica..."):
                try:
                    # Pasamos los parámetros de estrategia
    ...
    if st.sidebar.button("🔍 Escanear Mercado"):
        ...
        with st.spinner(f"Escaneando {chain}..."):
            try:
                # Pasamos los parámetros de estrategia al escáner masivo
    ...
    if st.session_state.scan_results is not None and not st.session_state.scan_results.empty:
        ...
        st.info(f"**Criterio de Veredicto:** Se compara si las FEES estimadas en **{dias_analisis} días** superan el RIESGO DE SALIDA (Max IL) de un rango de **{sd_mult} SD**.")
        ...
        st.dataframe(
            ...
            column_config={
                ...
                # Columnas de Datos Base
                col_apr: st.column_config.NumberColumn(format="%.1f%%"),
                "Volatilidad": st.column_config.NumberColumn(format="%.1f%%"),
                
                # NUEVAS COLUMNAS DE ESTRATEGIA (Calculadas en Analyzer)
                "Rango Est.": st.column_config.NumberColumn("Rango (±%)", format="%.1f%%"),
                "Est. Fees": st.column_config.NumberColumn(f"Fees Est. ({dias_analisis}d)", format="%.2f%%", help="Retorno estimado en % del capital si se mantiene en rango"),
                "Max IL": st.column_config.NumberColumn("Riesgo Salida", format="%.2f%%", help="Pérdida por Impermanent Loss si toca el límite"),
                
                "Margen": None, # Oculto, usado solo para ordenar/veredicto
...
# ==========================================
    ...
    # Fix Visual APR: Si viene como 0.8 (decimal), multiplicar por 100. Si viene 80.0, dejar igual.
    # Analyzer devuelve decimal (0.8), así que multiplicamos por 100 para mostrar %.
    val_apr = pool[col_apr_lab] * 100.0
    c3.metric("APR Media", f"{val_apr:.1f}%")
    
    # Volatilidad ya viene multiplicada por 100 en el analyzer
