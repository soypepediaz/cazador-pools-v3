...
if st.session_state.view == 'scanner':
    ...
    if st.sidebar.button("Analizar Pool Concreto"):
        if manual_address:
            ...
            # Usamos los sliders definidos más abajo
            # Nota: Streamlit renderiza secuencialmente, pero lee el valor actual del widget.
            # Si da error, usa defaults.
            dias_val = 7
            sd_val = 1.0
            
            with st.spinner("Analizando..."):
                try:
                    # Intentamos leer del estado si ya existen
                    # (Esto es un truco para compartir variables si están definidas después)
                    # Para seguridad, definimos los sliders ANTES.
                    pass 
                except: pass
            
    # MOVIDO ARRIBA PARA QUE ESTÉ DISPONIBLE
    st.sidebar.markdown("---")
    st.sidebar.header("⚙️ Estrategia")
    dias_analisis = st.sidebar.slider("Ventana Análisis (Días)", 3, 30, 7)
    sd_mult = st.sidebar.slider("Factor Rango (SD)", 0.1, 3.0, 1.0, step=0.1)

    # Lógica del botón manual (ahora que tenemos las variables)
    if manual_address and st.sidebar.button("Ejecutar Análisis Manual"):
        scanner = MarketScanner()
        with st.spinner("Analizando..."):
            try:
    ...
    if st.session_state.scan_results is not None and not st.session_state.scan_results.empty:
        ...
        st.info(f"**Criterio:** Fees Probables ({dias_analisis}d) vs Riesgo Salida ({sd_mult} SD).")
        ...
        with c1:
            ...
            def fmt(idx): 
                r = df_disp.iloc[idx]
                return f"{r['Par']} ({r['DEX']})"
            sel_idx = st.selectbox("Pool:", options=df_disp.index, format_func=fmt)
        with c2:
            st.write(""); st.write("")
            if st.button("Analizar ➡️"):
        ...
        if not address: st.error("Falta Address.")
            ...
            with st.spinner("Simulando..."):
                ...
                if df_res is not None and not df_res.empty:
                    ...
                    if auto_rebalance: st.info(f"🔄 **{meta['rebalances']} rebalanceos**.")
                    ...
                    with st.expander("Ver detalle"):
                        # FIX ROBUSTO: Solo mostramos columnas que existen
                        available_cols = df_res.columns.tolist()
                        target_cols = ["Date", "Price", "Range Min", "Range Max", "Range Width %", "APR Period", "Fees Period", "Valor Total"]
